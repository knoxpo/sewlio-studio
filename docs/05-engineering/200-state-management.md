...
This document defines how state management should be implemented in Sewlio Studio.
...
Engineering decisions for state management must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish immutable state
- Establish derived projections
- Establish event-fed caches
- Establish undo-safe transitions

---

# Definition

State Management is the engineering subsystem or policy area that governs immutable state and derived projections.
...
- Define package-level and service-level contracts for state management.
...
The state management implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The state management guidance can be implemented without contradicting the documented architecture.
