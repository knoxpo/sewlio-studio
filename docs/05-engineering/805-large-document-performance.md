# Engineering
## ENG-805 Large Document Performance
...
This document defines how large document performance should be implemented in Sewlio Studio.
...
Engineering decisions for large document performance must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish stitch-scale workloads
- Establish progressive loading
- Establish derived data reuse
- Establish degradation strategies

---

# Definition

Large Document Performance is the engineering subsystem or policy area that governs stitch-scale workloads and progressive loading.
...
- Define package-level and service-level contracts for large document performance.
...
The large document performance implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The large document performance guidance can be implemented without contradicting the documented architecture.
