...
This document defines how plugin packaging should be implemented in Sewlio Studio.
...
Engineering decisions for plugin packaging must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish manifest stability
- Establish sandboxable distribution
- Establish signature validation
- Establish compatibility metadata

---

# Definition

Plugin Packaging is the engineering subsystem or policy area that governs manifest stability and sandboxable distribution.
...
- Define package-level and service-level contracts for plugin packaging.
...
The plugin packaging implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The plugin packaging guidance can be implemented without contradicting the documented architecture.
