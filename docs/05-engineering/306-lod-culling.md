...
This document defines how lod culling should be implemented in Sewlio Studio.
...
Engineering decisions for lod culling must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish zoom-dependent detail
- Establish large document performance
- Establish predictable degradation
- Establish stable interactions

---

# Definition

LOD Culling is the engineering subsystem or policy area that governs zoom-dependent detail and large document performance.
...
- Define package-level and service-level contracts for lod culling.
...
The lod culling implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The lod culling guidance can be implemented without contradicting the documented architecture.
