...
This document defines how viewport engine should be implemented in Sewlio Studio.
...
Engineering decisions for viewport engine must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish camera transforms
- Establish view culling
- Establish hit-test correlation
- Establish precision mapping

---

# Definition

Viewport Engine is the engineering subsystem or policy area that governs camera transforms and view culling.
...
- Define package-level and service-level contracts for viewport engine.
...
The viewport engine implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The viewport engine guidance can be implemented without contradicting the documented architecture.
