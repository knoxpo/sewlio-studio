...
This document defines how rendering engine should be implemented in Sewlio Studio.
...
Engineering decisions for rendering engine must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish frame orchestration
- Establish scene preparation
- Establish overlay composition
- Establish fallback paths

---

# Definition

Rendering Engine is the engineering subsystem or policy area that governs frame orchestration and scene preparation.
...
- Define package-level and service-level contracts for rendering engine.
...
The rendering engine implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The rendering engine guidance can be implemented without contradicting the documented architecture.
