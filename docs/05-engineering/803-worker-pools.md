...
This document defines how worker pools should be implemented in Sewlio Studio.
...
Engineering decisions for worker pools must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish pool specialization
- Establish queue discipline
- Establish affinity policy
...
Worker Pools is the engineering subsystem or policy area that governs pool specialization and queue discipline.
...
- Define package-level and service-level contracts for worker pools.
...
The worker pools implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The worker pools guidance can be implemented without contradicting the documented architecture.
