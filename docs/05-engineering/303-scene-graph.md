...
This document defines how scene graph should be implemented in Sewlio Studio.
...
Engineering decisions for scene graph must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish render nodes
- Establish derived scene data
- Establish batching keys
- Establish selection annotations

---

# Definition

Scene Graph is the engineering subsystem or policy area that governs render nodes and derived scene data.
...
- Define package-level and service-level contracts for scene graph.
...
The scene graph implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The scene graph guidance can be implemented without contradicting the documented architecture.
