...
This document defines how file security should be implemented in Sewlio Studio.
...
Engineering decisions for file security must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish path validation
- Establish import sanitization
- Establish archive safety
- Establish secrets avoidance

---

# Definition

File Security is the engineering subsystem or policy area that governs path validation and import sanitization.
...
- Define package-level and service-level contracts for file security.
...
The file security implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The file security guidance can be implemented without contradicting the documented architecture.
