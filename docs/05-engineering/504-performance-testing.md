...
This document defines how performance testing should be implemented in Sewlio Studio.
...
Engineering decisions for performance testing must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish memory ceilings
- Establish large project scenarios

---

# Definition

Performance Testing is the engineering subsystem or policy area that governs benchmarks and frame-time budgets.
...
- Define package-level and service-level contracts for performance testing.
...
The performance testing implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The performance testing guidance can be implemented without contradicting the documented architecture.
