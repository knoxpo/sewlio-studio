...
This document defines how tracing should be implemented in Sewlio Studio.
...
Engineering decisions for tracing must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish cross-service spans
- Establish command/task tracing
- Establish AI span correlation
- Establish exportable traces

---

# Definition

Tracing is the engineering subsystem or policy area that governs cross-service spans and command/task tracing.
...
- Define package-level and service-level contracts for tracing.
...
The tracing implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The tracing guidance can be implemented without contradicting the documented architecture.
