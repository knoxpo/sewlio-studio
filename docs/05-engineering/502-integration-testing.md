...
This document defines how integration testing should be implemented in Sewlio Studio.
...
Engineering decisions for integration testing must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish cross-package behavior
- Establish pipeline execution
- Establish adapter contracts
- Establish realistic fixtures

---

# Definition

Integration Testing is the engineering subsystem or policy area that governs cross-package behavior and pipeline execution.
...
- Define package-level and service-level contracts for integration testing.
...
The integration testing implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The integration testing guidance can be implemented without contradicting the documented architecture.
