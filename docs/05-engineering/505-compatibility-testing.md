...
This document defines how compatibility testing should be implemented in Sewlio Studio.
...
Engineering decisions for compatibility testing must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish schema compatibility
- Establish plugin API compatibility
- Establish platform matrix coverage
- Establish machine output checks

---

# Definition

Compatibility Testing is the engineering subsystem or policy area that governs schema compatibility and plugin API compatibility.
...
- Define package-level and service-level contracts for compatibility testing.
...
The compatibility testing implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The compatibility testing guidance can be implemented without contradicting the documented architecture.
