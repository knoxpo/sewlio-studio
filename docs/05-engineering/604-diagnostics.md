...
This document defines how diagnostics should be implemented in Sewlio Studio.
...
Engineering decisions for diagnostics must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish self-checks
- Establish repair hints
- Establish user-visible errors
- Establish support artifacts

---

# Definition

Diagnostics is the engineering subsystem or policy area that governs self-checks and repair hints.
...
- Define package-level and service-level contracts for diagnostics.
...
The diagnostics implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The diagnostics guidance can be implemented without contradicting the documented architecture.
