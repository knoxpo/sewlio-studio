...
This document defines how testing strategy should be implemented in Sewlio Studio.
...
Engineering decisions for testing strategy must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish test pyramid alignment
- Establish deterministic fixtures
- Establish golden coverage
- Establish regression gates

---

# Definition

Testing Strategy is the engineering subsystem or policy area that governs test pyramid alignment and deterministic fixtures.
...
- Define package-level and service-level contracts for testing strategy.
...
The testing strategy implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The testing strategy guidance can be implemented without contradicting the documented architecture.
