...
This document defines how sandboxing should be implemented in Sewlio Studio.
...
Engineering decisions for sandboxing must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish process and capability containment
- Establish plugin isolation
...
Sandboxing is the engineering subsystem or policy area that governs process and capability containment and plugin isolation.
...
- Define package-level and service-level contracts for sandboxing.
...
The sandboxing implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The sandboxing guidance can be implemented without contradicting the documented architecture.
