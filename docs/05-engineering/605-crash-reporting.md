...
This document defines how crash reporting should be implemented in Sewlio Studio.
...
Engineering decisions for crash reporting must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish safe dumps
- Establish symbol resolution
- Establish privacy filters
- Establish recovery hooks

---

# Definition

Crash Reporting is the engineering subsystem or policy area that governs safe dumps and symbol resolution.
...
- Define package-level and service-level contracts for crash reporting.
...
The crash reporting implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The crash reporting guidance can be implemented without contradicting the documented architecture.
