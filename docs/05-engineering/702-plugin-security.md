...
This document defines how plugin security should be implemented in Sewlio Studio.
...
Engineering decisions for plugin security must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish signature checks
- Establish permission review
- Establish extension point hardening
- Establish revocation paths

---

# Definition

Plugin Security is the engineering subsystem or policy area that governs signature checks and permission review.
...
- Define package-level and service-level contracts for plugin security.
...
The plugin security implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The plugin security guidance can be implemented without contradicting the documented architecture.
