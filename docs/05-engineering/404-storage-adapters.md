...
This document defines how storage adapters should be implemented in Sewlio Studio.
...
Engineering decisions for storage adapters must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish local-first persistence
- Establish platform-specific backends
- Establish transaction boundaries
- Establish crash-safe behavior

---

# Definition

Storage Adapters is the engineering subsystem or policy area that governs local-first persistence and platform-specific backends.
...
- Define package-level and service-level contracts for storage adapters.
...
The storage adapters implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The storage adapters guidance can be implemented without contradicting the documented architecture.
