...
This document defines how shader system should be implemented in Sewlio Studio.
...
Engineering decisions for shader system must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish versioned shaders
- Establish specialization policy
...
Shader System is the engineering subsystem or policy area that governs versioned shaders and specialization policy.
...
- Define package-level and service-level contracts for shader system.
...
The shader system implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The shader system guidance can be implemented without contradicting the documented architecture.
