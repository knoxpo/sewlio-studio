...
This document defines how web architecture should be implemented in Sewlio Studio.
...
Engineering decisions for web architecture must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish browser sandbox constraints
- Establish WASM packaging
- Establish storage limits
- Establish reduced capability fallback

---

# Definition

Web Architecture is the engineering subsystem or policy area that governs browser sandbox constraints and WASM packaging.
...
- Define package-level and service-level contracts for web architecture.
...
The web architecture implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The web architecture guidance can be implemented without contradicting the documented architecture.
