...
This document defines how logging should be implemented in Sewlio Studio.
...
Engineering decisions for logging must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish structured events
- Establish redaction
- Establish sampling policy
- Establish developer readability

---

# Definition

Logging is the engineering subsystem or policy area that governs structured events and redaction.
...
- Define package-level and service-level contracts for logging.
...
The logging implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The logging guidance can be implemented without contradicting the documented architecture.
