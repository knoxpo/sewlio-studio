...
This document defines how desktop architecture should be implemented in Sewlio Studio.
...
Engineering decisions for desktop architecture must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish native shell integration
- Establish GPU access
- Establish filesystem permissions
- Establish multi-window orchestration

---

# Definition

Desktop Architecture is the engineering subsystem or policy area that governs native shell integration and GPU access.
...
- Define package-level and service-level contracts for desktop architecture.
...
The desktop architecture implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The desktop architecture guidance can be implemented without contradicting the documented architecture.
