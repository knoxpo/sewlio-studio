...
This document defines how render graph should be implemented in Sewlio Studio.
...
Engineering decisions for render graph must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish resource transitions
- Establish declarative dependencies
- Establish profiling hooks

---

# Definition

Render Graph is the engineering subsystem or policy area that governs pass ordering and resource transitions.
...
- Define package-level and service-level contracts for render graph.
...
The render graph implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The render graph guidance can be implemented without contradicting the documented architecture.
