...
This document defines how error handling should be implemented in Sewlio Studio.
...
Engineering decisions for error handling must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish typed failures
- Establish recovery paths
- Establish user-safe messages
...
Error Handling is the engineering subsystem or policy area that governs typed failures and recovery paths.
...
- Define package-level and service-level contracts for error handling.
...
The error handling implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The error handling guidance can be implemented without contradicting the documented architecture.
