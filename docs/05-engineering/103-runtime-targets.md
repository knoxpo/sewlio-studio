...
This document defines how runtime targets should be implemented in Sewlio Studio.
...
Engineering decisions for runtime targets must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish desktop runtime constraints
- Establish web runtime constraints
- Establish WASM/FFI boundaries
- Establish capability negotiation

---

# Definition

Runtime Targets is the engineering subsystem or policy area that governs desktop runtime constraints and web runtime constraints.
...
- Define package-level and service-level contracts for runtime targets.
...
The runtime targets implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The runtime targets guidance can be implemented without contradicting the documented architecture.
