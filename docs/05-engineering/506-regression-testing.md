...
This document defines how regression testing should be implemented in Sewlio Studio.
...
Engineering decisions for regression testing must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish bug lock-in tests
- Establish reproduction fixtures
- Establish history safety
- Establish release blocking

---

# Definition

Regression Testing is the engineering subsystem or policy area that governs bug lock-in tests and reproduction fixtures.
...
- Define package-level and service-level contracts for regression testing.
...
The regression testing implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The regression testing guidance can be implemented without contradicting the documented architecture.
