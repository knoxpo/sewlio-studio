...
This document defines how asset cache should be implemented in Sewlio Studio.
...
Engineering decisions for asset cache must preserve local-first behavior, command-driven mutation, event-derived updates, and scheduler-owned background work.
...
- Establish content-addressed reuse
- Establish eviction policy
- Establish cross-pipeline sharing
...
Asset Cache is the engineering subsystem or policy area that governs content-addressed reuse and eviction policy.
...
- Define package-level and service-level contracts for asset cache.
...
The asset cache implementation must fit within this layered model so that behavior remains testable and inspectable.
...
- The asset cache guidance can be implemented without contradicting the documented architecture.
