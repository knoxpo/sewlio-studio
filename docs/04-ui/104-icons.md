...
This document defines the icons architecture for Sewlio Studio.
...
The icons is command-driven and state-derived.
...
- Provide tool recognition
- Provide machine-safe metaphors
- Provide stroke consistency
- Provide state overlays

---

# Definition

Icons is the UI subsystem responsible for exposing tool recognition and machine-safe metaphors to end users.
...
- Render icons state using deterministic projections from the document model and runtime services.
...
The icons participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for icons.
...
- The icons surface can be implemented without introducing direct domain mutations from UI code.
