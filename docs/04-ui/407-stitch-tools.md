...
This document defines the stitch tools architecture for Sewlio Studio.
...
The stitch tools is command-driven and state-derived.
...
- Provide stitch-path authoring
- Provide sequence previews
- Provide density and underlay controls
- Provide rule-aware editing

---

# Definition

Stitch Tools is the UI subsystem responsible for exposing stitch-path authoring and sequence previews to end users.
...
- Render stitch tools state using deterministic projections from the document model and runtime services.
...
The stitch tools participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for stitch tools.
...
- The stitch tools surface can be implemented without introducing direct domain mutations from UI code.
