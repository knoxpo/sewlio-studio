...
This document defines the camera navigation architecture for Sewlio Studio.
...
The camera navigation is command-driven and state-derived.
...
- Provide pan zoom navigation
- Provide precision centering
- Provide input-device normalization
- Provide predictable coordinates

---

# Definition

Camera Navigation is the UI subsystem responsible for exposing pan zoom navigation and precision centering to end users.
...
- Render camera navigation state using deterministic projections from the document model and runtime services.
...
The camera navigation participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for camera navigation.
...
- The camera navigation surface can be implemented without introducing direct domain mutations from UI code.
