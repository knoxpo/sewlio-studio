...
This document defines the shape tools architecture for Sewlio Studio.
...
The shape tools is command-driven and state-derived.
...
- Provide primitive creation
- Provide parameter editing
- Provide constraint defaults
- Provide conversion to geometry

---

# Definition

Shape Tools is the UI subsystem responsible for exposing primitive creation and parameter editing to end users.
...
- Render shape tools state using deterministic projections from the document model and runtime services.
...
The shape tools participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for shape tools.
...
- The shape tools surface can be implemented without introducing direct domain mutations from UI code.
