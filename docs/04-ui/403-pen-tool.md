...
This document defines the pen tool architecture for Sewlio Studio.
...
The pen tool is command-driven and state-derived.
...
- Provide path creation
- Provide constraint hints
- Provide preview segments
- Provide cancel/commit behavior

---

# Definition

Pen Tool is the UI subsystem responsible for exposing path creation and constraint hints to end users.
...
- Render pen tool state using deterministic projections from the document model and runtime services.
...
The pen tool participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for pen tool.
...
- The pen tool surface can be implemented without introducing direct domain mutations from UI code.
