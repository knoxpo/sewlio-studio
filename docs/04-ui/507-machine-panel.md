...
This document defines the machine panel architecture for Sewlio Studio.
...
The machine panel is command-driven and state-derived.
...
- Provide machine profile selection
- Provide capability display
- Provide warning surfacing
...
Machine Panel is the UI subsystem responsible for exposing machine profile selection and capability display to end users.
...
- Render machine panel state using deterministic projections from the document model and runtime services.
...
The machine panel participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for machine panel.
...
- The machine panel surface can be implemented without introducing direct domain mutations from UI code.
