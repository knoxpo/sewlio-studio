...
docs/03-domain/threads/402-thread-colors.md
```
---

# Purpose

This document defines the color system architecture for Sewlio Studio.
...
The color system is command-driven and state-derived.
...
- Provide semantic color roles
- Provide thread-aware color previews
- Provide contrast guarantees
- Provide status signaling

---

# Definition

Color System is the UI subsystem responsible for exposing semantic color roles and thread-aware color previews to end users.
...
- Render color system state using deterministic projections from the document model and runtime services.
...
The color system participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for color system.
...
- The color system surface can be implemented without introducing direct domain mutations from UI code.
