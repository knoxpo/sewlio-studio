...
This document defines the canvas overview architecture for Sewlio Studio.
...
The canvas overview is command-driven and state-derived.
...
- Provide document visualization
- Provide overlay composition
- Provide high-precision interaction
- Provide GPU-backed presentation

---

# Definition

Canvas Overview is the UI subsystem responsible for exposing document visualization and overlay composition to end users.
...
- Render canvas overview state using deterministic projections from the document model and runtime services.
...
The canvas overview participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for canvas overview.
...
- The canvas overview surface can be implemented without introducing direct domain mutations from UI code.
