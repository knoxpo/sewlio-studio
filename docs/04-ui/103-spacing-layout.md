...
This document defines the spacing layout architecture for Sewlio Studio.
...
The spacing layout is command-driven and state-derived.
...
- Provide panel rhythm
- Provide canvas-first layout
- Provide resizable constraints
- Provide predictable scan paths

---

# Definition

Spacing Layout is the UI subsystem responsible for exposing panel rhythm and canvas-first layout to end users.
...
- Render spacing layout state using deterministic projections from the document model and runtime services.
...
The spacing layout participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for spacing layout.
...
- The spacing layout surface can be implemented without introducing direct domain mutations from UI code.
