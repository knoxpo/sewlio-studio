...
This document defines the shortcuts architecture for Sewlio Studio.
...
The shortcuts is command-driven and state-derived.
...
- Provide deterministic bindings
- Provide conflict resolution
- Provide workspace scopes
- Provide custom profiles

---

# Definition

Shortcuts is the UI subsystem responsible for exposing deterministic bindings and conflict resolution to end users.
...
- Render shortcuts state using deterministic projections from the document model and runtime services.
...
The shortcuts participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for shortcuts.
...
- The shortcuts surface can be implemented without introducing direct domain mutations from UI code.
