...
This document defines the workspaces architecture for Sewlio Studio.
...
The workspaces is command-driven and state-derived.
...
- Provide persona-specific layouts
- Provide task-oriented presets
- Provide persisted panel arrangements
- Provide recovery-safe layout state

---

# Definition

Workspaces is the UI subsystem responsible for exposing persona-specific layouts and task-oriented presets to end users.
...
- Render workspaces state using deterministic projections from the document model and runtime services.
...
The workspaces participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for workspaces.
...
- The workspaces surface can be implemented without introducing direct domain mutations from UI code.
