...
This document defines the color panel architecture for Sewlio Studio.
...
The color panel is command-driven and state-derived.
...
- Provide palette editing
- Provide semantic UI colors
- Provide thread color mapping
- Provide contrast cues

---

# Definition

Color Panel is the UI subsystem responsible for exposing palette editing and semantic UI colors to end users.
...
- Render color panel state using deterministic projections from the document model and runtime services.
...
The color panel participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for color panel.
...
- The color panel surface can be implemented without introducing direct domain mutations from UI code.
