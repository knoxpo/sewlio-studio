...
This document defines the text tool architecture for Sewlio Studio.
...
The text tool is command-driven and state-derived.
...
- Provide text placement
- Provide font preview
- Provide layout bounds
- Provide editable text objects

---

# Definition

Text Tool is the UI subsystem responsible for exposing text placement and font preview to end users.
...
- Render text tool state using deterministic projections from the document model and runtime services.
...
The text tool participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for text tool.
...
- The text tool surface can be implemented without introducing direct domain mutations from UI code.
