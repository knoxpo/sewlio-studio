...
This document defines the guides architecture for Sewlio Studio.
...
The guides is command-driven and state-derived.
...
- Provide non-destructive alignment aids
- Provide document persistence
- Provide manufacturing-safe positioning

---

# Definition

Guides is the UI subsystem responsible for exposing reference lines and non-destructive alignment aids to end users.
...
- Render guides state using deterministic projections from the document model and runtime services.
...
The guides participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for guides.
...
- The guides surface can be implemented without introducing direct domain mutations from UI code.
