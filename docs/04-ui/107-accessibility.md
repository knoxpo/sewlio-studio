...
This document defines the accessibility architecture for Sewlio Studio.
...
The accessibility is command-driven and state-derived.
...
- Provide screen-reader semantics
- Provide contrast and focus visibility
...
Accessibility is the UI subsystem responsible for exposing keyboard parity and screen-reader semantics to end users.
...
- Render accessibility state using deterministic projections from the document model and runtime services.
...
The accessibility participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for accessibility.
...
- The accessibility surface can be implemented without introducing direct domain mutations from UI code.
