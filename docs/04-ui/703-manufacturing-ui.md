...
This document defines the manufacturing ui architecture for Sewlio Studio.
...
The manufacturing ui is command-driven and state-derived.
...
- Provide machine/material alignment
- Provide cost/time summaries
- Provide operator sign-off

---

# Definition

Manufacturing UI is the UI subsystem responsible for exposing production readiness and machine/material alignment to end users.
...
- Render manufacturing ui state using deterministic projections from the document model and runtime services.
...
The manufacturing ui participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for manufacturing ui.
...
- The manufacturing ui surface can be implemented without introducing direct domain mutations from UI code.
