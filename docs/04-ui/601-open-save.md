...
This document defines the open save architecture for Sewlio Studio.
...
The open save is command-driven and state-derived.
...
- Provide dirty-state handling
- Provide recovery-safe saves
- Provide local-first persistence cues

---

# Definition

Open Save is the UI subsystem responsible for exposing recent files and dirty-state handling to end users.
...
- Render open save state using deterministic projections from the document model and runtime services.
...
The open save participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for open save.
...
- The open save surface can be implemented without introducing direct domain mutations from UI code.
