...
This document defines the typography architecture for Sewlio Studio.
...
The typography is command-driven and state-derived.
...
- Provide information hierarchy
- Provide dense workstation readability
- Provide measurement formatting
- Provide documentation tone inside UI

---

# Definition

Typography is the UI subsystem responsible for exposing information hierarchy and dense workstation readability to end users.
...
- Render typography state using deterministic projections from the document model and runtime services.
...
The typography participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for typography.
...
- The typography surface can be implemented without introducing direct domain mutations from UI code.
