...
This document defines the selection tool architecture for Sewlio Studio.
...
The selection tool is command-driven and state-derived.
...
- Provide hit testing
- Provide marquee behavior
- Provide modifier semantics
- Provide selection commit rules

---

# Definition

Selection Tool is the UI subsystem responsible for exposing hit testing and marquee behavior to end users.
...
- Render selection tool state using deterministic projections from the document model and runtime services.
...
The selection tool participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for selection tool.
...
- The selection tool surface can be implemented without introducing direct domain mutations from UI code.
