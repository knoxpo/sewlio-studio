...
This document defines the toolbars architecture for Sewlio Studio.
...
The toolbars is command-driven and state-derived.
...
- Provide high-frequency actions
- Provide state reflection
- Provide overflow behavior
- Provide customization boundaries

---

# Definition

Toolbars is the UI subsystem responsible for exposing high-frequency actions and state reflection to end users.
...
- Render toolbars state using deterministic projections from the document model and runtime services.
...
The toolbars participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for toolbars.
...
- The toolbars surface can be implemented without introducing direct domain mutations from UI code.
