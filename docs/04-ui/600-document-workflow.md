...
This document defines the document workflow architecture for Sewlio Studio.
...
The document workflow is command-driven and state-derived.
...
- Provide document lifecycle actions
- Provide save/import/export navigation
- Provide recovery-aware prompts
- Provide workflow continuity

---

# Definition

Document Workflow is the UI subsystem responsible for exposing document lifecycle actions and save/import/export navigation to end users.
...
- Render document workflow state using deterministic projections from the document model and runtime services.
...
The document workflow participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for document workflow.
...
- The document workflow surface can be implemented without introducing direct domain mutations from UI code.
