...
This document defines the selection system architecture for Sewlio Studio.
...
The selection system is command-driven and state-derived.
...
- Provide selection states
- Provide multi-object semantics
- Provide derived handles
- Provide history-safe interaction

---

# Definition

Selection System is the UI subsystem responsible for exposing selection states and multi-object semantics to end users.
...
- Render selection system state using deterministic projections from the document model and runtime services.
...
The selection system participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for selection system.
...
- The selection system surface can be implemented without introducing direct domain mutations from UI code.
