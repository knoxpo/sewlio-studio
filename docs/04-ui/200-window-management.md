...
This document defines the window management architecture for Sewlio Studio.
...
The window management is command-driven and state-derived.
...
- Provide multi-window project workflows
- Provide tool separation
- Provide state restoration
- Provide platform-native chrome

---

# Definition

Window Management is the UI subsystem responsible for exposing multi-window project workflows and tool separation to end users.
...
- Render window management state using deterministic projections from the document model and runtime services.
...
The window management participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for window management.
...
- The window management surface can be implemented without introducing direct domain mutations from UI code.
