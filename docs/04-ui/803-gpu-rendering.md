...
This document defines the gpu rendering architecture for Sewlio Studio.
...
The gpu rendering is command-driven and state-derived.
...
- Provide GPU-backed canvas presentation
- Provide overlay batching
- Provide texture policy
...
GPU Rendering is the UI subsystem responsible for exposing GPU-backed canvas presentation and overlay batching to end users.
...
- Render gpu rendering state using deterministic projections from the document model and runtime services.
...
The gpu rendering participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for gpu rendering.
...
- The gpu rendering surface can be implemented without introducing direct domain mutations from UI code.
