...
This document defines the performance architecture for Sewlio Studio.
...
The performance is command-driven and state-derived.
...
- Provide frame budget discipline
- Provide asynchronous state derivation
- Provide progressive disclosure
- Provide measurement-first tuning

---

# Definition

Performance is the UI subsystem responsible for exposing frame budget discipline and asynchronous state derivation to end users.
...
- Render performance state using deterministic projections from the document model and runtime services.
...
The performance participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for performance.
...
- The performance surface can be implemented without introducing direct domain mutations from UI code.
