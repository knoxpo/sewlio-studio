...
This document defines the ui overview architecture for Sewlio Studio.
...
The ui overview is command-driven and state-derived.
...
- Provide application shell composition
- Provide command-driven presentation model
- Provide state-derived interaction surfaces
- Provide desktop CAD ergonomics for embroidery

---

# Definition

UI Overview is the UI subsystem responsible for exposing application shell composition and command-driven presentation model to end users.
...
- Render ui overview state using deterministic projections from the document model and runtime services.
...
The ui overview participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for ui overview.
...
- The ui overview surface can be implemented without introducing direct domain mutations from UI code.
