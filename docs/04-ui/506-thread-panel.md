...
This document defines the thread panel architecture for Sewlio Studio.
...
The thread panel is command-driven and state-derived.
...
- Provide thread library browsing
- Provide brand and spool metadata
- Provide usage summaries
- Provide replacement workflows

---

# Definition

Thread Panel is the UI subsystem responsible for exposing thread library browsing and brand and spool metadata to end users.
...
- Render thread panel state using deterministic projections from the document model and runtime services.
...
The thread panel participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for thread panel.
...
- The thread panel surface can be implemented without introducing direct domain mutations from UI code.
