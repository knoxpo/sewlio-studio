...
This document defines the transform gizmos architecture for Sewlio Studio.
...
The transform gizmos is command-driven and state-derived.
...
- Provide move rotate scale controls
- Provide snap-aware transforms
- Provide selection-bound handles
- Provide preview vs commit separation

---

# Definition

Transform Gizmos is the UI subsystem responsible for exposing move rotate scale controls and snap-aware transforms to end users.
...
- Render transform gizmos state using deterministic projections from the document model and runtime services.
...
The transform gizmos participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for transform gizmos.
...
- The transform gizmos surface can be implemented without introducing direct domain mutations from UI code.
