...
This document defines the ai assistant ui architecture for Sewlio Studio.
...
The ai assistant ui is command-driven and state-derived.
...
- Provide chat and action surfaces
- Provide context disclosure
- Provide approval checkpoints
- Provide handoff to commands

---

# Definition

AI Assistant UI is the UI subsystem responsible for exposing chat and action surfaces and context disclosure to end users.
...
- Render ai assistant ui state using deterministic projections from the document model and runtime services.
...
The ai assistant ui participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for ai assistant ui.
...
- The ai assistant ui surface can be implemented without introducing direct domain mutations from UI code.
