...
This document defines the design guidelines architecture for Sewlio Studio.
...
The design guidelines is command-driven and state-derived.
...
- Provide professional workstation aesthetics
- Provide precision-first layout
- Provide domain-appropriate terminology
- Provide consistency rules

---

# Definition

Design Guidelines is the UI subsystem responsible for exposing professional workstation aesthetics and precision-first layout to end users.
...
- Render design guidelines state using deterministic projections from the document model and runtime services.
...
The design guidelines participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for design guidelines.
...
- The design guidelines surface can be implemented without introducing direct domain mutations from UI code.
