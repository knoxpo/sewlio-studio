...
This document defines the overlay rendering architecture for Sewlio Studio.
...
The overlay rendering is command-driven and state-derived.
...
- Provide selection overlays
- Provide diagnostic overlays
- Provide simulation overlays
...
Overlay Rendering is the UI subsystem responsible for exposing selection overlays and diagnostic overlays to end users.
...
- Render overlay rendering state using deterministic projections from the document model and runtime services.
...
The overlay rendering participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for overlay rendering.
...
- The overlay rendering surface can be implemented without introducing direct domain mutations from UI code.
