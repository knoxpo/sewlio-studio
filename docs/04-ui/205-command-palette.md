...
This document defines the command palette architecture for Sewlio Studio.
...
The command palette is command-driven and state-derived.
...
- Provide searchable actions
- Provide command aliases
- Provide parameterized execution
- Provide discoverability for experts

---

# Definition

Command Palette is the UI subsystem responsible for exposing searchable actions and command aliases to end users.
...
- Render command palette state using deterministic projections from the document model and runtime services.
...
The command palette participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for command palette.
...
- The command palette surface can be implemented without introducing direct domain mutations from UI code.
