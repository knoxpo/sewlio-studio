...
This document defines the assets panel architecture for Sewlio Studio.
...
The assets panel is command-driven and state-derived.
...
- Provide linked assets
- Provide preview caching
- Provide reference integrity
- Provide import handoff

---

# Definition

Assets Panel is the UI subsystem responsible for exposing linked assets and preview caching to end users.
...
- Render assets panel state using deterministic projections from the document model and runtime services.
...
The assets panel participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for assets panel.
...
- The assets panel surface can be implemented without introducing direct domain mutations from UI code.
