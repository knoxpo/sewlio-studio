...
This document defines the node tool architecture for Sewlio Studio.
...
The node tool is command-driven and state-derived.
...
- Provide path editing
- Provide bezier handle control
- Provide topology-safe edits
- Provide selection harmonization

---

# Definition

Node Tool is the UI subsystem responsible for exposing path editing and bezier handle control to end users.
...
- Render node tool state using deterministic projections from the document model and runtime services.
...
The node tool participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for node tool.
...
- The node tool surface can be implemented without introducing direct domain mutations from UI code.
