...
This document defines the animation architecture for Sewlio Studio.
...
The animation is command-driven and state-derived.
...
- Provide low-latency transitions
- Provide causal motion
- Provide selection feedback
- Provide non-disruptive status changes

---

# Definition

Animation is the UI subsystem responsible for exposing low-latency transitions and causal motion to end users.
...
- Render animation state using deterministic projections from the document model and runtime services.
...
The animation participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for animation.
...
- The animation surface can be implemented without introducing direct domain mutations from UI code.
