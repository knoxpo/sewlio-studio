...
This document defines the snapping architecture for Sewlio Studio.
...
The snapping is command-driven and state-derived.
...
- Provide deterministic snap targets
- Provide geometry and stitch anchors
- Provide preview hints
...
Snapping is the UI subsystem responsible for exposing deterministic snap targets and geometry and stitch anchors to end users.
...
- Render snapping state using deterministic projections from the document model and runtime services.
...
The snapping participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for snapping.
...
- The snapping surface can be implemented without introducing direct domain mutations from UI code.
