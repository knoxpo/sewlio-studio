...
The export ui is command-driven and state-derived.
...
- Provide target capability selection
- Provide validation before export
...
Export UI is the UI subsystem responsible for exposing target capability selection and validation before export to end users.
...
The export ui participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for export ui.
...
- The export ui surface can be implemented without introducing direct domain mutations from UI code.
