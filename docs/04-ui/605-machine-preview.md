...
The machine preview is command-driven and state-derived.
...
- Provide hoop positioning
- Provide needle path preview
- Provide machine warnings
- Provide compile result inspection
...
- Render machine preview state using deterministic projections from the document model and runtime services.
...
The machine preview participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for machine preview.
...
- The machine preview surface can be implemented without introducing direct domain mutations from UI code.
