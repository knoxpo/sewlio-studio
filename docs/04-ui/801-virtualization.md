...
This document defines the virtualization architecture for Sewlio Studio.
...
The virtualization is command-driven and state-derived.
...
- Provide large list virtualization
- Provide panel scalability
- Provide windowed rendering
- Provide stable selection anchoring

---

# Definition

Virtualization is the UI subsystem responsible for exposing large list virtualization and panel scalability to end users.
...
- Render virtualization state using deterministic projections from the document model and runtime services.
...
The virtualization participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for virtualization.
...
- The virtualization surface can be implemented without introducing direct domain mutations from UI code.
