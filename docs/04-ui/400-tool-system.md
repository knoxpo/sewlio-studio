...
This document defines the tool system architecture for Sewlio Studio.
...
The tool system is command-driven and state-derived.
...
- Provide tool lifecycle
- Provide shared interaction contracts
- Provide cursor ownership
- Provide command emission boundaries

---

# Definition

Tool System is the UI subsystem responsible for exposing tool lifecycle and shared interaction contracts to end users.
...
- Render tool system state using deterministic projections from the document model and runtime services.
...
The tool system participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for tool system.
...
- The tool system surface can be implemented without introducing direct domain mutations from UI code.
