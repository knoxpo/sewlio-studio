...
This document defines the themes architecture for Sewlio Studio.
...
The themes is command-driven and state-derived.
...
- Provide light and dark parity
- Provide color token stability
- Provide operator fatigue reduction
- Provide brand-neutral utility

---

# Definition

Themes is the UI subsystem responsible for exposing light and dark parity and color token stability to end users.
...
- Render themes state using deterministic projections from the document model and runtime services.
...
The themes participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for themes.
...
- The themes surface can be implemented without introducing direct domain mutations from UI code.
