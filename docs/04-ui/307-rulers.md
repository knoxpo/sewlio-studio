...
This document defines the rulers architecture for Sewlio Studio.
...
The rulers is command-driven and state-derived.
...
- Provide edge measurement
- Provide unit display
- Provide guide creation affordances
- Provide cursor correlation

---

# Definition

Rulers is the UI subsystem responsible for exposing edge measurement and unit display to end users.
...
- Render rulers state using deterministic projections from the document model and runtime services.
...
The rulers participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for rulers.
...
- The rulers surface can be implemented without introducing direct domain mutations from UI code.
