...
This document defines the grids architecture for Sewlio Studio.
...
The grids is command-driven and state-derived.
...
- Provide measurement grids
- Provide zoom-adaptive density
- Provide unit-aware spacing
- Provide low-cost rendering

---

# Definition

Grids is the UI subsystem responsible for exposing measurement grids and zoom-adaptive density to end users.
...
- Render grids state using deterministic projections from the document model and runtime services.
...
The grids participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for grids.
...
- The grids surface can be implemented without introducing direct domain mutations from UI code.
