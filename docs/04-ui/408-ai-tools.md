...
This document defines the ai tools architecture for Sewlio Studio.
...
The ai tools is command-driven and state-derived.
...
- Provide AI proposal surfaces
- Provide approval workflows
- Provide explainability affordances
- Provide safe command submission

---

# Definition

AI Tools is the UI subsystem responsible for exposing AI proposal surfaces and approval workflows to end users.
...
- Render ai tools state using deterministic projections from the document model and runtime services.
...
The ai tools participates in this loop without bypassing command validation, event propagation, or scheduler-owned background work.
...
- Plugin-specific extensions for ai tools.
...
- The ai tools surface can be implemented without introducing direct domain mutations from UI code.
